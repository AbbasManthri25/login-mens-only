        ...
        <ul className="hidden md:flex items-center gap-10">
          {LINKS.map((link) => (
            <li key={link.href}>
              <Link
                to={link.href}
                className="relative text-sm font-medium text-ink hover:text-ink/70 transition-colors after:content-[''] after:absolute after:left-0 after:-bottom-1 after:h-[1.5px] after:w-0 after:bg-gold-dark after:transition-all hover:after:w-full"
              >
                {link.label}
              </Link>
            </li>
              ...
              {LINKS.map((link) => (
                <li key={link.href}>
                  <Link
                    ...
                    className="block py-3 text-base text-ink border-b border-ink/10"
                  >
                    {link.label}
                  </Link>
                </li>
